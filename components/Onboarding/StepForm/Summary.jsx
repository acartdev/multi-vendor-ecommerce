...
export default function Summary({ farmerId }) {
  ...
  const submitData = async (e) => {
    e.preventDefault();
    setLoading(true);
    const code = generateUserCode("LFF", `${onboardingFormData.firstName}`);
    const data = {
      ...onboardingFormData,
      code: code,
      userId: farmerId,
  ...
  return (
    <div>
      ...
      <div className="">Here are Details</div>
      <div className="mt-4 flex justify-between items-center w-full ">
        <button onClick={handlePrevious} type="button" disabled={loading} className="btn">
          <ChevronLeft />
        ...
        {loading ? (
          ...
          <button
            onClick={submitData}
            type="button"
            className="btn uppercase text-slate-200  "
          >
            Submit Data
            <CircleChevronRight className="text-green-400 " />
          </button>
        )}
      </div>
    </div>
